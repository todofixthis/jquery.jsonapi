...
- `ok` indicates that the request succeeded (HTTP status `200 OK`).
- `fail` indicates that something was wrong with the request (e.g., a bad
    parameter value; HTTP status `400 Bad Request`).

## Detail
The contents of the `detail` element depend upon the `status` of the response:

- `ok`:  contains any values specified by the server to send back to
  the requester, such as query results, a status value, etc.
- `fail`:  contains a single sub-array called `errors` which contains the
  error messages generated by the server.
